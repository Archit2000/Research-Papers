...
# In[37]:
def visited(alpha_param,beta_param,gamma_param,lambda_param,delta_param):
    df = pd.read_csv(readings_path+'/'+'parameter_readings.csv')
    df = df[df['alpha_param']==alpha_param]
    df = df[df['beta_param']==beta_param]
    df = df[df['gamma_param']==gamma_param]
    df = df[df['lambda_param']==lambda_param]
    df = df[df['delta_param']==delta_param]
    return len(df.index)
...
for alpha_param in [100,10,1,0.1,0.01,0.001,0.0001,0.00001,0.000001]:
    for beta_param in [100,10,1,0.1,0.01,0.001,0.0001,0.00001,0.000001]:
        for gamma_param in [100,10,1,0.1,0.01,0.001,0.0001,0.00001,0.000001]:
            for lambda_param in [100,10,1,0.1,0.01,0.001,0.0001,0.00001,0.000001]:
                for delta_param in [100,10,1,0.1,0.01,0.001,0.0001,0.00001,0.000001]:
                    try:
                        if not visited(alpha_param,beta_param,gamma_param,lambda_param,delta_param):
